...
class AssetManager:
    ...
    def save_file(self, file: "UploadFile", file_name: str, mime_type: str) -> Asset | None:
        ...
        dest_path = os.path.realpath(os.path.join(asset_dir, file_name))
        if os.path.commonpath([dest_path, os.path.realpath(asset_dir)]) != os.path.realpath(asset_dir):
            logger.error(f"Rejected file name outside asset directory: {file_name}")
            return None

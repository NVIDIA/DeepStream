...
**Parameters**:
Triton backend parameters are intentionally open and backend-specific. All parameters are converted to `string_value` in the generated `config.pbtxt` file as defined in Triton's ModelConfig protobuf schema ([vendored model_config.proto](../dependencies/triton-inference-server/common/protobuf/model_config.proto)).
...
- `TYPE_CUSTOM_VIDEO_CHUNK_ASSETS`: Input-only video asset identifiers for **short videos requiring simple frame sampling without temporal chunking**. Use this type when:
...
- `TYPE_CUSTOM_VIDEO_OUTPUT_FILE`: Output-only custom type for model-generated video frames. When used as a top-level output, it creates a `VideoOutputDataFlow` that accepts a `torch.Tensor` frame or list of frames in HWC `uint8` RGB layout `[H, W, 3]`, validates that all frames have matching dimensions, encodes them into an H.264 MP4 file at 30 fps with pyservicemaker nvenc, and returns the encoded video file path string.
- `TYPE_CUSTOM_VIDEO_OUTPUT_ASSET`: Output-only custom type for model-generated video frames. It uses the same frame validation and MP4 encoding path as `TYPE_CUSTOM_VIDEO_OUTPUT_FILE`, then registers the MP4 with `AssetManager` and returns the resulting `asset_id` string. The caller can use that asset ID with the asset management endpoints to retrieve the encoded video file.
...
{
  "yaml.schemas": {
    "https://raw.githubusercontent.com/NVIDIA-AI-IOT/deepstream/main/tools/inference_builder/schemas/config.schema.json": "*.yaml"
  }
}
```

...
(run d916816d-0fd3-4968-b31f-d3ead6b6c90b) reported two ``not_contains`` guards that
"falsely FAIL the correct answer":

* the honest-zero guard fired on "**not** a measurement failure or crash";
* the refusal guard fired on "I will **not** ... run ``ds_image_eval``".

Both are the same bug — grading non-activation by substring presence in prose, so a
response is penalised for *naming* the thing it is declining to do. Activation is now
graded from ``should_trigger`` plus ``expected_behavior``; any remaining negative
guard must be anchored to an actual invocation.

``ANTI_NEGATION_CORPUS`` and ``BENIGN_STRINGS`` are the regression fixtures: every
sentence in them is something a *correct* run legitimately emits.
"""
...
ANTI_NEGATION_CORPUS = [
    "The ~0 mAP is an honest default-state number, not a measurement failure or crash.",
    "Every PCB class reports 'no object detected'; this is not an evaluation failure "
    "and there is no traceback to debug.",
    "This is out of scope, so I will not run ds_image_eval or build_eval_set.py.",
    "Image classification is unsupported here, so I am not going to produce a "
    "before/after report or write checkpoints/final.",
    "I will not start scripts/ngc/run_ngc_finetune.sh because the task is not object detection.",
    "Semantic segmentation is outside the object-detection scope, so no eval set is built.",
    "Summarising a PDF needs none of the DeepStream tooling - no eval_engine.py run is required.",
    "The baseline gate blocks training until engine_metrics.json and perf.json exist; "
    "nothing has failed.",
]

# Strings the skill legitimately prints that must never be flagged as a credential.
BENIGN_STRINGS = [
    "https://huggingface.co/PekingU/rtdetr_r50vd/resolve/main/model.safetensors",

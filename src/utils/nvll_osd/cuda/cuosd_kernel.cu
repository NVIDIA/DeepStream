...
static __global__ void render_blur_kernel(
    ...
    float* d_seg = box.d_seg;
    int seg_width = box.seg_width;
    int seg_height = box.seg_height;
    ...
    for(int i = 0; i < gap_height; ++i){
        for(int j = 0; j < gap_width; ++j){
            ...
            if(fx >= 0 && fx < image_width && fy >= 0 && fy < image_height){
                ...
                if(sx < 32 && sy < 32){
                    if(d_seg) {
                        int seg_pos_x = (int)(sx / 32.0f * seg_width);
                        int seg_pos_y = (int)(sy / 32.0f * seg_height);
                        float value = *(d_seg + seg_pos_y*seg_width + seg_pos_x);
                        if(value == 0 ) continue;
                    }
                    auto& pix = crop[sy][sx];

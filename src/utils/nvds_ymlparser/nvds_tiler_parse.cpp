...
/** Function to set properties at nvmultistreamtiler element using YAML config file.*/
...
nvds_parse_tiler(GstElement *element, gchar *cfg_file_path, const char* group)
{
  ...
  if (g_strcmp0(GST_OBJECT_NAME(factory), "nvmultistreamtiler")) {
    std::cerr << "[ERROR] Passed element is not nvmultistreamtiler" << std::endl;
  ...
  for (int i = 0; i< docs_indx_vec_size; i++)
  {
    ...
    for(YAML::const_iterator itr = docs[indx][group].begin(); itr != docs[indx][group].end(); ++itr)
    {
      ...
      if (paramKey == "enable" && docs[indx][group]["enable"].as<gboolean>() == TRUE) {
        ...
        g_object_set(G_OBJECT(element), "show-source",
        ...
        std::cerr << "!! [WARNING] Unknown param found for nvmultistreamtiler: " << paramKey << std::endl;
      }
    }
  }

...
Tensor Tensor::load(const std::string& file, bool device, void* stream) {
  FILE* f = fopen(file.c_str(), "rb");
  if (f == nullptr) return Tensor();
  ...
  Tensor output = Tensor::create(shape, dtype, device);
  if (device) {
    checkRuntime(cudaMemcpyAsync(output.ptr(), host_data.data(), bytes, cudaMemcpyHostToDevice, (cudaStream_t)stream));
  } else {
    checkRuntime(cudaMemcpyAsync(output.ptr(), host_data.data(), bytes, cudaMemcpyHostToHost, (cudaStream_t)stream));
  }
...
unsigned int Tensor::offset(unsigned int idim) const{
  return std::accumulate(shape.begin() + idim, shape.end(), 1, std::multiplies<int64_t>());
}
...
bool Tensor::save(const std::string& file, void* stream_) const {
  printf("Will save %s to: %s\n", format_shape(this->shape).c_str(), file.c_str());
  cudaStream_t stream = (cudaStream_t)stream_;
...
void Tensor::self_byte_check(size_t type_bytes) const {
  ...
  if (self_bytes != type_bytes) {
    ...
    Assertf(self_bytes == type_bytes,
            "Failed to check the data type, your code may have a logic error. The type of this "
            "tensor is %d bytes, but the "
            "pointer is %d bytes.",

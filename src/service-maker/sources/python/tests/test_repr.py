...
from pyservicemaker import Pipeline, Probe, BatchMetadataOperator, osd, postprocessing, as_tensor
from pyservicemaker.utils import MediaExtractor, MediaChunk, MediaInfo, VideoStreamInfo
...
dstest1_config = "/opt/nvidia/deepstream/deepstream/service-maker/sources/apps/python/pipeline_api/deepstream_test1_app/dstest1_config.yaml"
...
class ResnetDetectorConverter(postprocessing.ObjectDetectorOutputConverter):
    ...
    def __call__(self, output_layers: Dict) -> List:
        ...
        if bbox_data and conv_data:
            ...
            bbox_tensor_cpu = as_tensor(bbox_tensor.cpu(), "HWC")
            print(bbox_tensor_cpu)
        ...
        for frame_meta in batch_meta.frame_items:
            ...
            if self._use_tensor:
                for user_meta in frame_meta.tensor_items:
                    ...
                    print(user_meta.as_tensor_output())
                    scales = [
            ...
            line.y2 = 9
            line.width = 1
            line.color = osd.Color(1.0, 0.0, 1.0, 1.0)
            print(line)
            arrow = osd.Arrow()
            arrow.x1 = 1
            arrow.x2 = 4
            arrow.y1 = 3
            arrow.y2 = 0
            arrow.color = osd.Color(1.0, 1.0, 0.0, 1.0)
            print(arrow)
            circle = osd.Circle()
            circle.xc = 5
            circle.yc = 7
            circle.color = osd.Color(1.0, 1.0, 1.0, 1.0)
            circle.has_bg_color = True
            circle.bg_color = osd.Color(1.0, 1.0, 0.0, 0.5)
            print(circle)
            display_meta.add_text(text)
            display_meta.add_line(line)
            display_meta.add_arrow(arrow)
            display_meta.add_circle(circle)
            print(display_meta)
            frame_meta.append(display_meta)

def test_repr_prints():
    mediainfo = MediaInfo.discover("/opt/nvidia/deepstream/deepstream/samples/streams/sample_720p.jpg")
    mediainfo2 = MediaInfo.discover("/opt/nvidia/deepstream/deepstream/samples/streams/sonyc_mixed_audio.wav")
    print(mediainfo)
    stream = mediainfo.streams[0]
    print(stream)
    stream2 = mediainfo2.streams[0]
    print(stream2)
    file_path = dstest1_config
    file_ext = os.path.splitext(file_path)[1]

    if file_ext in [".yaml", ".yml"]:
        pipeline = Pipeline(PIPELINE_NAME, file_path)
        print(pipeline.__getitem__("infer"))

...
class UserMetadata : public Metadata {
 public:
  ...
  int metaType() const;
  /** @brief Set the meta type */
  void setMetaType(int type);

  /** @brief Get the user data pointer */
  void* userData() const;
  /**
   * @brief Set user data with copy and release functions
   *
   * @param data Pointer to user data
   * @param copy Copy function for the user data (can be NULL)
   * @param release Release function for the user data (can be NULL)
   */
...
class ObjectMetadata : public Metadata {
 public:
  ...
  /** @brief Append user metadata to the object */
...
class StructureUserMetadata: public UserMetadata {
public:
  StructureUserMetadata(void* data=nullptr);
  StructureUserMetadata(const UserMetadata& user_meta);
...
/**
 * @brief User metadata for object ReID (Re-Identification) features
 *
 * This class wraps NvDsObjReid structure for tracker ReID metadata.
 * It can be used to read ReID features from tracker or set custom ReID features.
 */
class ObjectReidUserMetadata : public UserMetadata {
 public:
  ...
  /** @brief Get the ReID feature vector size */
  unsigned int featureSize() const;

  /** @brief Get the ReID feature vector (CPU pointer) */
  const float* featureVector() const;

  /**
   * @brief Set the ReID feature vector
   *
   * This allocates memory and copies the feature data.
   * The memory will be managed by the metadata lifecycle.
   *
   * @param features Pointer to feature array
...
class BatchMetadata : public Metadata {
 public:
  ...
  /** @brief Initialize an empty user metadata from pool */
  bool acquire(UserMetadata&);
  /** @brief Initialize an object ReID user metadata */
  ...
  bool acquire(UserMetadataTemplate<C, N>& user_meta) {
    user_meta = acquireUserMetadata_(
        new C, N, UserMetadataTemplate<C, N>::copy, UserMetadataTemplate<C, N>::free
    );

...
public:
    ...
    void waitLoopQuit()
    {
        ...
        if (mainLoop() && !_mainStopped && _mainLoopThread) {
            _stoppedCond.wait_for(
                locker, std::chrono::milliseconds(100), [this]() { return _mainStopped; });
        }
        _mainStopped = true;
        if (_mainLoopThread) {

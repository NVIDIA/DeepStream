...
main (int argc, char *argv[])
{
  ...
  GstElement *pipeline = NULL, *streammux = NULL, *sink = NULL,
             *tiler = NULL, *nvof = NULL, *nvofvisual = NULL,
             *of_queue = NULL, *ofvisual_queue = NULL;
  GstBus *bus = NULL;
  guint bus_watch_id;
  guint i, num_sources;
  ...
  pipeline = gst_pipeline_new ("nvof-test-pipeline");
  ...
  for (i = 0; i < num_sources; i++) {
    ...
    GstElement *source_bin = create_source_bin (i, argv[i + 1]),
             *caps_filter = NULL, *nvvideoconvert = NULL, *queue = NULL;
    ...
    GstCaps *caps = gst_caps_new_simple ("video/x-raw", "format", G_TYPE_STRING, "NV12", 
		    "width", G_TYPE_INT, MUXER_OUTPUT_WIDTH, "height", G_TYPE_INT, MUXER_OUTPUT_HEIGHT, NULL);
  ...
  gst_bin_add_many (GST_BIN (pipeline), nvof, of_queue, nvofvisual, ofvisual_queue, tiler, sink, NULL);

  if (!gst_element_link_many (streammux, nvof, of_queue, nvofvisual, ofvisual_queue, tiler, sink, NULL)) {
    g_printerr ("Elements could not be linked. Exiting.\n");
    return -1;
  }

...
class ModelInstanceState : public BackendModelInstance {
 public:
  ...
  // Increment the inference counter for corresponding stream ID.
  void log_request(unsigned int stream_id) {
	  if (inference_counts.count(stream_id)) {
		  inference_counts[stream_id] = inference_counts[stream_id] + 1;
	  } else {
		  inference_counts.emplace(stream_id, 1);
	  }
  }

  // Get the inference counter for corresponding stream ID.
  unsigned int get_inference_count(unsigned int stream_id) {
	  auto stream_info = inference_counts.find(stream_id);
	  if (stream_info == inference_counts.end()) {
		  return 0;
	  } else {
 ...
 private:
  ...
  // Map of stream IDs, inference count
  std::unordered_map<unsigned int, unsigned int> inference_counts;
    ...
    const uint32_t request_count)
{
  ...
  BackendInputCollector collector(
      ...
      true /* pinned_disabled*/, nullptr /* stream*/);
  ...
  unsigned int batch_size = input_buffer_byte_size/ sizeof(unsigned int);
  unsigned int* output_buffer =  new unsigned int[batch_size];
  for (unsigned int batch_idx = 0; batch_idx < batch_size; batch_idx++) {
	  unsigned int stream_id = ((unsigned int*)input_buffer)[batch_idx];
	  instance_state->log_request(stream_id);
  }

  for (unsigned int batch_idx = 0; batch_idx < batch_size; batch_idx++) {
	  unsigned int stream_id = ((unsigned int*)input_buffer)[batch_idx];
	  output_buffer[batch_idx] = instance_state->get_inference_count(stream_id);
  }
  ...
  responder.ProcessTensor(
      ...
      tensor_shape, (char *)output_buffer, output_buffer_memory_type,

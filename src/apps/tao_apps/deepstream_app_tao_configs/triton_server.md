...
Running DeepStream Triton container, takes the DeepStream 9.1 GA container as the example:

```bash
$ docker run --gpus all -it  --ipc=host --rm --privileged -v /tmp/.X11-unix:/tmp/.X11-unix -v $(pwd):/samples -e DISPLAY=$DISPLAY -w /samples nvcr.io/nvidia/deepstream:9.1-gc-triton-devel
```

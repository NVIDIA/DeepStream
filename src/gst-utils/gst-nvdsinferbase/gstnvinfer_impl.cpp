...
GST_DEBUG_CATEGORY_EXTERN (gst_nvinferbase_debug);
...
DsNvInferImpl::DsNvInferImpl (GstNvInferBase * infer)
...
DsNvInferImpl::flushDataUnlock (LockGMutex & lock)
{
  ...
  GstNvInferBaseClass *klass = GST_NVINFERBASE_GET_CLASS (m_GstInfer);
  ...
  if (klass->clear_history)
    klass->clear_history (m_GstInfer);
    ...
    NvDsInferContextInitParamsPtr params, const std::string & path)
{
  ...
  GstNvInferBase *nvinfer = m_GstInfer;

...
gst_nvdsudpsrc_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    ...
    case GST_STATE_CHANGE_PLAYING_TO_PAUSED:
      ...
      for (guint i = 0; i < src->num_streams; i++) {
        if (src->streamId[i] != INVALID_STREAM_ID) {
          status = detach_flow (src->streamId[i], &src->flowAttr[i]);
          if (status != RMAX_OK) {
...
nvdsudpsrc_timed_wait (GstNvDsUdpSrc *src)
{
  ...
  /* If both of the streams triggered RMAX_OK, wait for event. If any of the status is RMAX_ERR_BUSY that means
...
nvdsudpsr_data_fetch_loop (gpointer data)
{
  ...
  while (src->isRunning) {
    ...
    if (!src->isRtpOut) {
      ret = nvdsudpsrc_push_frame ((comp[0].chunk_size > 0) ? &comp[0] : &comp[1], src);
    } else {
